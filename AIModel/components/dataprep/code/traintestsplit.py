...
def main():
    ...
    for dataset in datasets:
        flower_images = glob(dataset + "/*.jpg")
        print(f"Found {len(flower_images)} images for {dataset}")
        random.seed(SEED)
        random.shuffle(flower_images)
        amount_of_test_images = math.ceil(len(flower_images) * train_test_split_factor)

        flower_test_images = flower_images[:amount_of_test_images]
        flower_training_images = flower_images[amount_of_test_images:]

        testing_datapaths.extend(flower_test_images)
        training_datapaths.extend(flower_training_images)

        print(testing_datapaths[:5])

        for img in flower_test_images:
            with open(img, "rb") as f:
                with open(os.path.join(args.testing_data_output, os.path.basename(img)), "wb") as f2:
                    f2.write(f.read())

        for img in flower_training_images:
            with open(img, "rb") as f:
